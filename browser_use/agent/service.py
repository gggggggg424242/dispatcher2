...
from typing import Any, Callable, Dict, List, Optional, Type, TypeVar, Generic, TYPE_CHECKING
...
class Agent:
    def __init__(
        ...
        controller: "Controller[BrowserContext]" | None = None,
        ...
        # Initialize controller first so other components can use it
        self.controller: "Controller[BrowserContext]" = controller if controller is not None else Controller.create(BrowserContext)
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None):
        ...
        logger.info(f'\U0001F4CD Step {self.n_steps}')
        ...
        if len(result) > 0 and result[-1].is_done:
            logger.info(f'\U0001F4C4 Result: {result[-1].extracted_content}')
    ...
    async def _handle_step_error(self, error: Exception) -> list[ActionResult]:
        ...
        if isinstance(error, (ValidationError, ValueError)):
            ...
            if 'Max token' in str(error):
                logger.warning("Token limit exceeded, retrying with truncated history")
                return [ActionResult(error="Token limit exceeded", include_in_memory=True)]
    ...
    def _log_response(self, response: AgentOutput) -> None:
        ...
        logger.debug(f'\U0001F916 {emoji} Page summary: {response.current_state.page_summary}')
        logger.info(f'{emoji} Eval: {response.current_state.evaluation_previous_goal}')
        logger.info(f'\U0001F9E0 Memory: {response.current_state.memory}')
        logger.info(f'\U0001F3AF Next goal: {response.current_state.next_goal}')

        for i, action in enumerate(response.action):
            logger.info(f'\U0001F6E0️  Action {i + 1}/{len(response.action)}: {action.model_dump_json(exclude_unset=True)}')
    ...
    def _log_agent_run(self) -> None:
        """Log the agent run"""
        logger.info(f'\U0001F680 Starting task: {self.task}')
    ...
    async def run(self, max_steps: int = 100) -> AgentHistoryList:
        ...
        try:
            ...
            if state:
                self._make_history_item(model_output, state, result if result else [])
    ...
    async def _execute_history_step(self, history_item: AgentHistory, delay_between_actions: float = 2.0) -> list[ActionResult]:
        """
        Execute a single step from history with the given delay between actions.

        Args:
            history_item: The history item containing actions to execute
            delay_between_actions: Delay between actions in seconds
        ...
        if not history_item.model_output or not history_item.model_output.action:
            return [ActionResult(error="No actions to execute")]

        # Get current state to update element indices
        state = await self.browser_context.get_state()

        # Copy actions to avoid modifying the original
        actions = []
        for action in history_item.model_output.action:
            ...
            # Find element in current DOM if it's an element-based action
            historical_element = history_item.state.interacted_element[0] if history_item.state.interacted_element else None

            # Update indices based on current state
            updated_action = await self._update_action_indices(historical_element, action, state)
            if updated_action:
                actions.append(updated_action)
            else:
                logger.warning(f"Element not found in current DOM, skipping action: {action}")
    ...
    def create_history_gif(
        ...
        # Create frames for each step
            if hasattr(self.planner_llm, 'model'):
                self.planner_model_name = self.planner_llm.model
                self.planner_model_name = self.planner_llm.model
            else:
                self.planner_model_name = 'Unknown'
    ...
    def pause(self):
        logger.info('\U0001F504 pausing Agent ')
        self._paused = True
